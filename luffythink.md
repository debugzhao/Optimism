...
学习主题：OP Stack、Superchain

- OP Stack 是以太坊通过模块化进行扩展的基本愿景的颂歌。Optimism 正在建立一个完全开源的 rollup 链生态系统，而 OP Stack 是建立这个愿景的基础。但是，不仅仅是 rollup，OP Stack 还可以被利用来建立治理和身份模块，让开发者有能力轻松地从头设计他们的链。**OP 链不是 Cosmos 应用链。它们也不是单链。它是一个由相同技术标准合并的超级链。**扩展阅读：https://www.aicoin.com/zh-Hans/article/324693
