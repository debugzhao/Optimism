...
- Rollups 将交易数据存储在主链上，而主链的存储和验证能力比链下环境更可靠。
- 通过压缩技术，交易数据的大小被大幅减少（例如使用 Merkle 树或聚合签名），在不牺牲安全性的前提下提升了性能。


### **4. 无需逐笔处理交易**

- Plasma 需要在欺诈证明中逐笔回溯和验证有争议的交易。
- Rollups 则直接通过压缩后的交易数据和证明文件快速验证状态，无需处理所有中间交易，减少了验证工作量。


### **5. 数据压缩优化**

- Rollups 使用数据压缩技术（如批量处理、Merkle 树），将大量交易打包成一个批次，再提交到主链。
- 这种“卷起”的方式极大减少了主链需要处理的交易量，同时确保了所有交易数据可用。


### **6. 用户体验更好**

- 在 Rollups 中，大部分交易默认被视为有效（Optimistic Rollups）或已通过证明验证（ZK Rollups），因此用户无需等待冗长的验证过程或挑战期。
- Plasma 则需要用户主动参与验证过程，复杂性更高，延迟更长。
